...
import sys
# import os
# os.chdir('J:/Research/research/Misc&archived_projects/COVID/Model/v3(movIni_best10)/homework2')
# %load_ext autoreload
# %autoreload 2
...
def main(BH, BU, nsim):
    ...
    pid = 0
    t = 0
    for i in sys_column:  # get the corresponding states
    ...
    for t in range(1, horizon - 1):
        ...
        for i in pop:
            ...
            if i.c_state == 'ER':
                ...
                x = [np.insert(
                    x, 1, 1 / (sys_report.system_stat['H'][t] / Beds[bed_map['H']] + .01)),
                    np.insert(
                        x, 1, 1 / (sys_report.system_stat['U'][t] / Beds[bed_map['U']] + .01)),
                    np.insert(
                        x, 1, Beds[bed_map['DD']]),
                    np.insert(
                        x, 1, Beds[bed_map['DA']]),
                ...
                i.update(n_state)
            # sys_report.update_t(i.c_state, pp[1])
            # print(prev_state, n_state, sys_report.system_stat[n_state][t])
            sys_report.update_state(t, i.history.loc[t])
        # print(sys_report.system_stat)
