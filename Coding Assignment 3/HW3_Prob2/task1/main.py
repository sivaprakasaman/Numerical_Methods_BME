...
import sys
# import os
# os.chdir('J:\Research\\research'
#          '\\Misc&archived_projects\\COVID\\Model'
#          '\\v3(movIni_best10)\\Hw3prob\\task1\\')
...
def main(l1, l2, l3):
    ...
    sys_report.system_stat
    """Start simulation time at t=1
    """
    pid = 0
    t = 0
    np.random.seed(479060)
    ...
    for t in range(1, horizon + 1):
        ...
        for j in range(1, 3):
            ...
            for i in range(int(adm.iloc[t - 1, j])):
                ...
                history = pd.DataFrame(
                    0, index=np.arange(horizon + 2), columns=column)
                pj = Patient(pid, t, covid, 'E', history.copy())
        ...
        out1 = sys_report.system_stat['H'] + sys_report.system_stat['HV']
        out2 = sys_report.system_stat['I'] + sys_report.system_stat['IV']
        r1 = out1.iloc[1:, ].rolling(14).sum().dropna().reset_index(drop=True)
        r2 = out2.iloc[1:, ].rolling(14).sum().dropna().reset_index(drop=True)
    return [r1, r2]
...
if __name__ == '__main__':
    ...
    l3 = float(sys.argv[3])
    o = main(l1, l2, l3)
    sys.stdout.write(' '.join(map(str, o[0].astype(int))))
    sys.stdout.write(' ')
    sys.stdout.write(' '.join(map(str, o[1].astype(int))))
