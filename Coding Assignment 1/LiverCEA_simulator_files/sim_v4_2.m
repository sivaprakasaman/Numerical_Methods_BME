...
function [seed,cost,death_ev,death_hcc,death_cp]=sim_v4_2(seed,...
...
p_hccpreo);%EV test
       ...
       elseif prior_ev(1)==3
           ...
           prior_ev(1)=3;
           prior_ev(2)=clock;
           cost_egd2=c_egd/inflation^ceil(clock/12);
           if treat==1
                 state(ti-1,4)=1;
...
%      if surgery(2)==0
    ...
    p_hccpreo);%HCC test
       ...
       % No surveillance,then the fastest way to detect maybe when bleeding
       elseif (state(ti-1,2)==2&&state(ti-1,3)==1&&prior_ev(1)==3)
%            if prior_ev(1)==1
...
%           end
    ...
    if condition==1
        ...
        elseif hidden_hcc(ti-1)==1 && state(ti-1,1)==1%compensated cirrhosis mortality
            [seed,u]=u16807d(seed);
            if u<=mor_cp_p
                death_cp=1;
                break
...
cost=cost_bb+cost_ebl+cost_trans+cost_rsct;
